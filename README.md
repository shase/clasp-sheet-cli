...
このツールは、短く壊れにくいローカル自動化を最優先に設計しています。

- 境界の固定
  - Spreadsheet の業務ロジックは Apps Script 側に集約し、CLI は入出力と実行制御だけを担当する
- 実行方式の交換可能性
  - コマンド実装は抽象実行インターフェースに依存し、clasp run から MCP や HTTP へ差し替え可能にする
- 運用コスト最小化
  - Google Sheets REST API クライアントと追加認証実装を避け、開発者の初期セットアップ負荷を減らす
- 可観測性と自己診断
  - doctor と status で環境問題を先に検出し、エラー時は復旧手順を提示する
- ローカルファースト
  - npm 公開前提にせず、個人開発や自動化ジョブで即利用できる構成を維持する
