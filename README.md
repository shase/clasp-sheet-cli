...
境界: 業務ロジックは右側の Apps Script に集約。左側の CLI は入出力・設定・HTTP 実行・認証トークンの付与のみ。`clasp` バイナリは実行時には登場せず、その**ログイン成果物（`~/.clasprc.json`）だけ**を認証に再利用します。
...
`clasp` はここ（デプロイ時）で使い、実行時には使いません。
